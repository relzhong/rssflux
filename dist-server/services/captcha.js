...
export function hashCaptchaAnswer(answer) {
    return crypto
        .createHash("sha256")
        .update(answer.trim().toUpperCase())
        .digest("hex");
}
export function generateRandomCaptcha(length = 4) {
    let text = "";
...
export class CaptchaService {
    ...
    async createChallenge() {
        ...
        await this.db.query(`INSERT INTO auth_captcha (id, answer_hash, expires_at)
       VALUES ($1, $2, $3)`, [id, answerHash, expiresAt]);
        return { id, image: svg };
    }
    async verifyAndConsumeChallenge(id, answer) {
        if (!id || !answer)
            return false;
        const providedHash = hashCaptchaAnswer(answer);
        // Fetch and mark challenge as used in a single atomic query
        const res = await this.db.query(`UPDATE auth_captcha
       SET used_at = now()
       WHERE id = $1 AND used_at IS NULL
       RETURNING answer_hash, expires_at, used_at`, [id]);
        if (res.rowCount === 0) {
            return false;
        }
    ...
    async cleanupExpired() {
        await this.db.query(`DELETE FROM auth_captcha

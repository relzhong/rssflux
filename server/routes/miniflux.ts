...
import type { MinifluxService } from "../services/miniflux.js";

interface MinifluxRoutesOptions {
  minifluxService: MinifluxService;
}

export const minifluxRoutes: FastifyPluginAsync<MinifluxRoutesOptions> = async (
  fastify,
  opts
) => {
  ...
  fastify.all("/*", async (req, reply) => {
    ...
    try {
      const response = await minifluxService.proxyRequest(
        subPath,
        method,
        headers,
        body,

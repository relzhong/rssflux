...
describe("Article Summary Integration (HTTP Seam)", () => {
  ...
  it("generates and persists summary on demand, then reuses it on subsequent requests", async () => {
    ...
    mockMiniflux.articles.set(101, {
      ...
      comments_url: "",
      author: "Fastify Team",
      content: "<p>Fastify is an efficient web framework for Node.js focused on performance.</p>",
    ...
    expect(genData.summary).toContain("Detailed summary of How Fastify Scales");
    ...
    expect(mockAi.generateCallCount).toBe(1); // Still 1! Deduplication preserved
  });

  it("regenerates summary when article content changes", async () => {
    // 1. Initial article
    mockMiniflux.articles.set(102, {
      id: 102,
      user_id: 1,
      feed_id: 1,
    ...
    await app.inject({
      method: "POST",
      url: "/api/summary/102/generate",
      cookies: { session: sessionCookie },
    });
    expect(mockAi.generateCallCount).toBe(1);

    // 2. Update article content upstream
    mockMiniflux.articles.set(102, {
      id: 102,
      user_id: 1,
      feed_id: 1,
      title: "Version 2 Title (Updated)",
      url: "https://example.com/v1",
      comments_url: "",
      author: "Author",
      content: "<p>Updated content with significant new developments</p>",
      published_at: new Date().toISOString(),
      created_at: new Date().toISOString(),
      status: "unread",
      starred: false,
    ...
    // 3. Generate again -> content hash differs -> triggers fresh AI generation
    const res = await app.inject({
      method: "POST",
      url: "/api/summary/102/generate",
      cookies: { session: sessionCookie },
    ...
    expect(mockAi.generateCallCount).toBe(2);
    const data = JSON.parse(res.body);
    expect(data.title).toBe("Version 2 Title (Updated)");
  });
});
